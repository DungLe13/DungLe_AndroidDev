...
public class NoteDetailActivity extends BaseActivity
    ...
    private void setValues(String id) {
        ...
        String selection = NotesContract.NoteColumns.NOTE_ID + "=" + id;
        Cursor cursor = getContentResolver().query(r, projection, selection, null, null);
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    String title = cursor.getString(cursor.getColumnIndex(NotesContract.NoteColumns.NOTES_TITLE));
                    String description = cursor.getString(cursor.getColumnIndex(NotesContract.NoteColumns.NOTES_DESCRIPTION));
                    String time = cursor.getString(cursor.getColumnIndex(NotesContract.NoteColumns.NOTES_TIME));
                    String date = cursor.getString(cursor.getColumnIndex(NotesContract.NoteColumns.NOTES_DATE));
                    String image = cursor.getString(cursor.getColumnIndex(NotesContract.NoteColumns.NOTES_IMAGE));
                    int storageSelection = cursor.getInt(cursor.getColumnIndex(NotesContract.NoteColumns.NOTES_IMAGE_STORAGE_SELECTION));
    ...
    private void setValues(Note note) {
        ...
        if (note.getType() == AppConstant.LIST) {
            mIsList = true;
        }
        mTitleEditText.setText(title);
    ...
    private void saveInDropBox() {
        ...
        ContentValues values = createContentValues(AppConstant.NOTE_PREFIX + GDUT.time2Titl(null),
                AppConstant.DROP_BOX_SELECTION, true);
    ...
    private void saveInGoogleDrive() {
        ...
        ContentValues values = createContentValues(AppConstant.NOTE_PREFIX + GDUT.time2Titl(null) + AppConstant.JPG,
                AppConstant.GOOGLE_DRIVE_SELECTION, true);
        createNoteAlarm(values, insertNote(values));
    }
    ...
    private void editForSaveInDropbox() {
        ...
        if (mIsImageSet) {
            ...
            values.put(NotesContract.NoteColumns.NOTES_IMAGE, fileName);
...
//                            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == CAMERA_REQUEST && resultCode == RESULT_OK) {
            ...
            Uri tmpUri = getImageUri(getApplicationContext(), photo);
            File finalFile = new File(getRealPathFromURI(tmpUri));
            ...
            if (resultCode == RESULT_OK) {
                ...
                String[] fileFathColumn = {MediaStore.Images.Media.DATA};
                Cursor cursor = getContentResolver().query(selectedImage, fileFathColumn, null, null, null);
                cursor.moveToFirst();
                int columnIndex = cursor.getColumnIndex(fileFathColumn[0]);
    ...
    private ContentValues createContentValues(String noteImage, int storageSelection, boolean isSave) {
        ...
        ContentValues values = new ContentValues();
        values.put(NotesContract.NoteColumns.NOTES_TITLE, mTitleEditText.getText().toString());
        values.put(NotesContract.NoteColumns.NOTES_DATE, sDateTextView.getText().toString());
        values.put(NotesContract.NoteColumns.NOTES_TIME, sTimeTextView.getText().toString());
        if (mIsImageSet || isSave) {
            values.put(NotesContract.NoteColumns.NOTES_IMAGE, noteImage);
        }
        values.put(NotesContract.NoteColumns.NOTES_IMAGE_STORAGE_SELECTION, storageSelection);
        ...
        values.put(NotesContract.NoteColumns.NOTES_TYPE, type);
        values.put(NotesContract.NoteColumns.NOTES_DESCRIPTION, description);
        return values;
    }
    ...
    private void updateNote(ContentValues values) {
        ContentResolver contentResolver = getContentResolver();
        Uri uri = Uri.parse(NotesContract.BASE_CONTENT_URI + "/notes");
        String selection = NotesContract.NoteColumns.NOTE_ID + " = " + mId;
        contentResolver.update(uri, values, selection, null);
    }

    private void createNoteAlarm(ContentValues values, int id) {
        if (!sTimeTextView.getText().toString().equals(AppConstant.NO_TIME)) {
            Note note = new Note(values.getAsString(NotesContract.NoteColumns.NOTES_TITLE),
                    values.getAsString(NotesContract.NoteColumns.NOTES_DESCRIPTION),
                    values.getAsString(NotesContract.NoteColumns.NOTES_DATE),
                    values.getAsString(NotesContract.NoteColumns.NOTES_TIME),
                    id,
                    values.getAsInteger(NotesContract.NoteColumns.NOTES_IMAGE_STORAGE_SELECTION),
                    values.getAsString(NotesContract.NoteColumns.NOTES_TYPE));
            note.setImagePath(values.getAsString(NotesContract.NoteColumns.NOTES_IMAGE));
