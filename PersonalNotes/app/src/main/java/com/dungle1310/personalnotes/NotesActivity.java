...
public class NotesActivity extends BaseActivity implements
    ...
    private void setUpRecyclerView() {
        ...
        GridLayoutManager linearLayout = new GridLayoutManager(getApplicationContext(), 1);
                ...
                threads[threadCounter] = new Thread(new Runnable() {
                    ...
                    public void run() {
                        do {
                            ...
                            if (gfs.size() > 0) {
                                byte[] imageByte = GDActions.read(gfs.get(0).id, 0);
    ...
    private Drawable getImageFromDropbox(DropboxAPI<?> mApi, String mPath, String filemame) {
        FileOutputStream fos = null;
        Drawable drawable;
        String cachePath = getApplicationContext().getCacheDir().getAbsolutePath() + "/" + filemame;
        ...
        if (cacheFile.exists()) {
            ...
            try {
                DropboxAPI.Entry dirEntry = mApi.metadata(mPath, 1000, null, true, null);
                ...
                for (DropboxAPI.Entry entry : dirEntry.contents) {
                    if (entry.thumbExists) {
                        if (entry.fileName().startsWith(filemame)) {
                            thumbs.add(entry);
                        }
                    }
                }
                ...
                if (thumbs.size() == 0) {
                    ...
                    DropboxAPI.Entry entry = thumbs.get(0);
                    ...
                    mApi.getThumbnail(path, fos, DropboxAPI.ThumbSize.BESTFIT_960x640, DropboxAPI.ThumbFormat.JPEG, null);
    ...
    private boolean checkUserAccount() {
        ...
        if (email == null) {
            if (account == null) {
                ...
                Intent accountIntent = AccountPicker.newChooseAccountIntent(account, null, new String[]{GoogleAuthUtil.GOOGLE_ACCOUNT_TYPE},
...
//                TextView tx = (TextView) first.getChildAt(1);
//                CheckBox cb = (CheckBox) first.getChildAt(0);
//                listDescription = description + tx.getText().toString() + cb.isChecked() + "%";
//            }
